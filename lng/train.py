...
def main():
    ...
    parser.add_argument('--data_dir_raw', type=str, default=CONFIG['data_dir_raw'],
                        help="Path to the raw data directory.")
    parser.add_argument('--impl', type=str, default=CONFIG["impl"], help="Which linear implementation to use.")
    ...
    if os.path.isdir(checkpoint_dir):
        print(f"Error: Checkpoint directory '{checkpoint_dir}' already exists.", file=sys.stderr)
        print("Please specify a new --run_name or delete the existing folder to proceed.", file=sys.stderr)
        sys.exit(1)
    ...
    logger.log("\n--- Initializing Model ---")
    assert CONFIG["impl"] in ["naive", "tangent", "correction", "chen"]
    if CONFIG["manifold"] == "Euclidean":
        ...
        model = HSeq2SeqTransformer(
            ...
            src_vocab_size=vocab_size, tgt_vocab_size=vocab_size,
            dim_feedforward=CONFIG["dim_feedforward"], dropout=CONFIG["dropout"],
            manifold=manifold, impl="naive",
        ).to(device)
        count_parameters(model, logger)
        optimizer = hypll.optim.RiemannianAdam(model.parameters(), lr=1.0, betas=(0.9, 0.98), eps=1e-12,
                                               correction=True)
    else:
        model = HSeq2SeqTransformer(
            num_encoder_layers=CONFIG["num_encoder_layers"],
            ...
            src_vocab_size=vocab_size, tgt_vocab_size=vocab_size,
            dim_feedforward=CONFIG["dim_feedforward"], dropout=CONFIG["dropout"],
            manifold=manifold, impl=CONFIG["impl"]
        ).to(device)
        count_parameters(model, logger)
        optimizer = hypll.optim.RiemannianAdam(model.parameters(), lr=1.0, betas=(0.9, 0.98), eps=1e-8,
                                               correction=False)
