...
class Euclidean(Manifold):
    ...
    def expmap(self, v: TangentTensor) -> ManifoldTensor:
        ...
        return ManifoldTensor(
            data=new_tensor, manifold=self, man_dim=v.broadcasted_man_dim
        )
    ...
    def logmap(self, x: Optional[ManifoldTensor], y: ManifoldTensor) -> TangentTensor:
        ...
        return TangentTensor(
            data=new_tensor, manifold_points=x, manifold=self, man_dim=dim
        )
    ...
    def transp(self, v: TangentTensor, y: ManifoldTensor) -> TangentTensor:
        ...
        return TangentTensor(
            data=new_tensor, manifold_points=y, manifold=self, man_dim=dim
        )
    ...
    def hyperplane_dists(
        self, x: ManifoldTensor, z: ManifoldTensor, r: Optional[Tensor]
    ) -> Tensor:
    ...
    def frechet_variance(
        ...
        if mu is None:
            ...
            if mu.man_dim != x.man_dim:
                raise ValueError(
                    "Input tensor and mean do not have matching manifold dimensions"
                )
            n = 1
            for bd in batch_dim:
                n *= x.size(dim=bd)
            return (x.tensor - mu.tensor).pow(2).sum(dim=batch_dim, keepdim=keepdim) / (
                n - 1
            )
