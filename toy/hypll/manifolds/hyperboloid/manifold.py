...
class Hyperboloid(Manifold):
    ...
    def multiheadattention(
        ...
        n_sqrt = torch.tensor(
            query.shape[-1] - 1
        ).sqrt()  # Space dimension of the hyperboloid (time is not counted).
        scores = -squared_lorentzian_distance(query.tensor, key.tensor, self.c()) / n_sqrt

        if attn_mask is not None:
            # The input attn_mask has -inf for masked positions. We create a
            # boolean mask where True indicates a position to be masked.
            bool_attn_mask = (attn_mask == -torch.inf)

            # --- FIX ---
            # Get the shape of the scores tensor to expand the mask correctly.
            batch_size, num_heads, seq_len, _ = scores.shape
            # Expand the mask to (B, H, L, L) to ensure it applies correctly.
            expanded_mask = bool_attn_mask.unsqueeze(0).unsqueeze(0).expand(batch_size, num_heads, seq_len, seq_len)
            scores = scores.masked_fill(expanded_mask, float("-inf"))

        if key_padding_mask is not None:
            scores = scores.masked_fill(key_padding_mask.unsqueeze(1).unsqueeze(2), float('-inf'))
        attn_weights = torch.nn.functional.softmax(scores, dim=-1)
        mu = self.midpoint(x=value, w=attn_weights, num_heads=num_heads)  # (B, L, D + 1)
    ...
    def euc_to_tangent(
        ...
        """
        # tangent_vectors = euc_to_tangent(
        #     x=x.tensor, u=u.tensor, dim=x.man_dim
        # )
    ...
    # def euc_to_tangent(self, x: ManifoldTensor, u: ManifoldTensor, dim: int = -1) -> Tensor:
    ...
    #     )
        ...
        # minkowski_dot = -x.tensor[..., 0] * u.tensor[..., 0] + torch.sum(
        #     x.tensor[..., 1:] * u.tensor[..., 1:], dim=-1, keepdim=True
        # )
        #
        # print(minkowski_dot.shape)
        #
        # # Project u onto the tangent space by adding the component along x.
        # dta = u.tensor + minkowski_dot * x.tensor
        # return TangentTensor(data=dta, manifold=self, manifold_points=x, dim=-1)
