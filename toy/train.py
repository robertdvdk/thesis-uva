...
MODIFIED FOR DEBUGGING - TASK 1: DIRECT COPY
The decoder input is identical to the target output. This is the simplest
possible test to see if the model can learn to copy. This setup assumes
the model is NOT internally applying a causal mask.
"""
...
def evaluate(model: nn.Module, criterion: nn.Module,
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            # --- TASK 1: DIRECT COPY SETUP ---
            # The decoder input and the target for the loss are identical.
            tgt = src.clone()

            tgt_input = tgt.clone()
            tgt_out = tgt.clone()

            # Replace padding in the loss target with the ignore_index
            tgt_out[tgt_out == pad_token_id] = criterion.ignore_index

            # The decoder's padding mask should ignore padding tokens in its input.
...
def main():
    ...
    logger.log("\n--- Freezing Euclidean layers (classifier and embeddings) ---")
    for name, param in model.named_parameters():
        if 'generator' in name or 'emb' in name:
            param.requires_grad = False
            logger.log(f"Froze parameter: {name}")

    trainable_params = [p for p in model.parameters() if p.requires_grad]
    count_parameters(model, logger)
    optimizer = hypll.optim.RiemannianAdam(trainable_params, lr=1.0, betas=(0.9, 0.98), eps=1e-8)
    ...
    logger.log("\n--- Starting Training ---")
    logger.log("--- RUNNING DEBUG TASK 1: DIRECT COPY ---")
    ...
    while g_step < max_steps and not training_complete:
        for batch in train_loader:
            ...
            logits = model(src, tgt_input, src_padding_mask, tgt_padding_mask)
            print(logits.max(dim=-1)[0].max())
            print(logits.mean(dim=-1).mean())
            # --- END TASK 1 SETUP ---
