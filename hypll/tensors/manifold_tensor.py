...
class ManifoldTensor:
    ...
    def __init__(
        ...
        """
        self.tensor = (
            data if isinstance(data, Tensor) else tensor(data, requires_grad=True)
        )
    ...
    def __getitem__(self, *args):
        ...
        if len(args) != 1:
            raise ValueError(
                "No support for slicing with these arguments. If you think there should be "
                "support, please consider opening a issue on GitHub describing your case."
            )
        ...
        if isinstance(args[0], Tensor) and args[0].dtype == long:
            ...
            return ManifoldTensor(
                data=new_tensor, manifold=self.manifold, man_dim=new_man_dim
            )
        ...
        for arg in arg_list:
            ...
            if counter == 0:
                if isinstance(arg, int) or isinstance(arg, list):
                    raise ValueError(
                        "Attempting to slice into the manifold dimension, but this is not a "
                        "valid operation"
                    )
        ...
        return ManifoldTensor(
            data=new_tensor, manifold=self.manifold, man_dim=output_man_dim
        )
    ...
    def cpu(self) -> ManifoldTensor:
        ...
        return ManifoldTensor(
            data=new_tensor, manifold=self.manifold, man_dim=self.man_dim
        )
    ...
    def detach(self) -> ManifoldTensor:
        ...
        return ManifoldTensor(
            data=detached, manifold=self.manifold, man_dim=self.man_dim
        )
    ...
    def reshape(self, shape) -> ManifoldTensor:
        """Returns a reshaped version of the manifold tensor."""
        reshaped_tensor = self.tensor.reshape(shape)
        return ManifoldTensor(
            data=reshaped_tensor,
    ...
    def squeeze(self, dim=None):
        ...
        if dim is None:
            ...
            new_man_dim = self.man_dim - sum(
                self.size(d) == 1 for d in range(self.man_dim)
            )
        else:
    ...
    def stack(self, y, dim=None):
        tensor1 = self.tensor
        tensor2 = y.tensor

        return ManifoldTensor(
            data=torch.stack([tensor1, tensor2], dim=dim), manifold=self.manifold
        )
